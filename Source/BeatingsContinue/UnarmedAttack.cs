...
internal class UnarmedAttack
{
    private static string defaultLabel => "Unarmed Attack";
    private static string defaultDesc => "Punch Them!";
    private static JobDef jobDef => BeatingsDefsOf.jobDefUnarmed;
    ...
    public static Gizmo GetGizmo(Pawn pawn)
    {
        ...
        command_Target.action = delegate(LocalTargetInfo target)
        {
            foreach (var item in Find.Selector.SelectedObjects.Where(
                         x => x is Pawn { IsColonistPlayerControlled: true }).Cast<Pawn>())
            {
    ...
    public static Action GetAttackAction(Pawn pawn, LocalTargetInfo target, out string failStr)
    {
        ...
        else
        {
                ...
                !pawn.InSameExtraFaction(target2, ExtraFactionType.MiniFaction))
            {
                return delegate
                {
                    if (!InteractionUtility.TryGetRandomVerbForSocialFight(pawn, out _))
                    {
                        return;
                    }
